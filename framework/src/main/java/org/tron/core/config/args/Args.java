...
public class Args extends CommonParameter {
    ...
    if (StringUtils.isNoneBlank(PARAMETER.privateKey)) {
        ...
        if (PARAMETER.isWitness()
        && CollectionUtils
            .isEmpty(localWitnesses.getPrivateKeys())) {
        ...
        if (config.hasPath(Constant.GENESIS_BLOCK)) {
            ...
            PARAMETER.genesisBlock
                .setParentHash(config.getString(Constant.GENESIS_BLOCK_PARENTHASH));
        ...
        PARAMETER.nodeListenPort =
            config.hasPath(Constant.NODE_LISTEN_PORT) ? config.getInt(Constant.NODE_LISTEN_PORT)
                : 0;
    ...
    PARAMETER.nodeP2pPingInterval =
        ...
        PARAMETER.nodeP2pVersion =
            config.hasPath(Constant.NODE_P2P_VERSION) ? config.getInt(Constant.NODE_P2P_VERSION)
                : 0;
        ...
        PARAMETER.rpcOnPBFTPort =
            config.hasPath(Constant.NODE_RPC_PBFT_PORT) ? config
                .getInt(Constant.NODE_RPC_PBFT_PORT) : 50071;
    ...
    PARAMETER.rpcThreadNum =
        ...
        PARAMETER.tcpNettyWorkThreadNum =
            config.hasPath(Constant.NODE_TCP_NETTY_WORK_THREAD_NUM)
                ? config.getInt(Constant.NODE_TCP_NETTY_WORK_THREAD_NUM) : 0;

        PARAMETER.udpNettyWorkThreadNum =
            config.hasPath(Constant.NODE_UDP_NETTY_WORK_THREAD_NUM)
                ? config.getInt(Constant.NODE_UDP_NETTY_WORK_THREAD_NUM) : 1;

        if (StringUtils.isEmpty(PARAMETER.trustNodeAddr)) {
            PARAMETER.trustNodeAddr =
                config.hasPath(Constant.NODE_TRUST_NODE) ? config
                    .getString(Constant.NODE_TRUST_NODE) : null;
        }
    ...
    PARAMETER.walletExtensionApi =
        ...
        PARAMETER.activeConnectFactor =
            config.hasPath(Constant.NODE_ACTIVE_CONNECT_FACTOR) ? config
                .getDouble(Constant.NODE_ACTIVE_CONNECT_FACTOR) : 0.1;
    ...
    PARAMETER.maxTransactionPendingSize = config.hasPath(Constant.NODE_MAX_TRANSACTION_PENDING_SIZE)
        ...
        PARAMETER.minEffectiveConnection =
            config.hasPath(Constant.NODE_RPC_MIN_EFFECTIVE_CONNECTION)
    ...
    PARAMETER.allowTvmAssetIssue =
            config.hasPath(Constant.COMMITTEE_ALLOW_TVM_ASSETISSUE) ? config
                    .getInt(Constant.COMMITTEE_ALLOW_TVM_ASSETISSUE) : 0;initBackupProperty(config);
    ...
    private static Account createAccount(final ConfigObject asset) {
        ...
        account
            .setAddress(Commons.decodeFromBase58Check(asset.get("address").unwrapped().toString()));
    ...
    private static EventPluginConfig getEventPluginConfig(final com.typesafe.config.Config config) {
        ...
        if (config.hasPath(Constant.EVENT_SUBSCRIBE_TOPICS)) {
            List<TriggerConfig> triggerConfigList = config
                .getObjectList(Constant.EVENT_SUBSCRIBE_TOPICS)
                .stream()
    ...
    long toBlockLong = 0;
        ...
        List<String> rootContractList = config.getStringList(Constant.EVENT_SUBSCRIBE_ROOT_CONTRACT)
            .stream().filter(StringUtils::isNotEmpty).collect(Collectors.toList());

        addressList = addressList.stream().filter(StringUtils::isNotEmpty)
            .collect(Collectors.toList());
        filter.setContractAddressList(addressList);
        filter.setRootContractList(rootContractList);
        filter.setRootContractInitTimes(
            config.getLong(Constant.EVENT_SUBSCRIBE_ROOT_CONTRACT_INIT_TIMES));
    ...
    private static void externalIp(final com.typesafe.config.Config config) {
        ...
        .getString(Constant.NODE_DISCOVERY_EXTERNAL_IP).trim().isEmpty()) {
            if (PARAMETER.nodeExternalIp == null) {
                logger
                    .info("External IP wasn't set, using checkip.amazonaws.com to identify it...");
                BufferedReader in = null;
                try {
                    in = new BufferedReader(new InputStreamReader(
                        new URL(Constant.AMAZONAWS_URL).openStream()));
                    PARAMETER.nodeExternalIp = in.readLine();
                    if (PARAMETER.nodeExternalIp == null || PARAMETER.nodeExternalIp.trim()
                        .isEmpty()) {
