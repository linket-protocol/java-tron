...
public class VM {
  ...
  private static final BigInteger _32_ = BigInteger.valueOf(32);
    ...
    /**
     * Utility to calculate new total memory size needed for an operation. <br/> Basically just
     * offset + size, unless size is 0, in which case the result is also 0.
     *
    ...
    public void step(Program program) {
        ...
        throw Program.Exception.invalidOpCode(program.getCurrentOp());
            ...
            switch (op) {
                ...
                case EXTCODECOPY:
                    energyCost =
                        energyCosts.getEXT_CODE_COPY() + calcMemEnergy(energyCosts, oldMemSize,
                ...
                case CALLTOKEN:
                    ...
                    adjustedCallEnergy = program
                        .getCallEnergy(op, callEnergyWord, getEnergyLimitLeft);
                ...
                case CREATE2:
                    ...
                    energyCost += calcMemEnergy(energyCosts, oldMemSize,
                        memNeeded(stack.get(stack.size() - 2), stack.get(stack.size() - 3)), 0, op);
                    energyCost +=
                        DataWord.sizeInWords(codeSize.intValueSafe()) * energyCosts.getSHA3_WORD();
                ...
                case LOG4:
                    ...
                    if (program.getEnergyLimitLeft().value().compareTo(dataCost) < 0) {
                        throw new OutOfEnergyException(
                            ...
                            program.getEnergyLimitLeft().longValueSafe());
                    }
            ...
            switch (op) {
                ...
                case RETURNDATACOPY: {
                    ...
                    if (msgData == null) {
                        throw new Program.ReturnDataCopyIllegalBoundsException(dataOffsetData,
                            lengthData,
        ...
        case POP: {
                ...
                case LOG4: {
                    ...
                    byte[] data = program
                        .memoryChunk(memStart.intValueSafe(), memOffset.intValueSafe());
                ...
                case STATICCALL: {
                    ...
                    if (program.isStaticCall() && (op == CALL || op == CALLTOKEN) && !value
                        .isZero()) {
                        throw new Program.StaticCallModificationException();
                    }
                    ...
                    if (logger.isDebugEnabled()) {
                        ...
                        logger.debug(ENERGY_LOG_FORMATE,
                ...
                case REVERT: {
                    ...
                    byte[] hReturn = program
                        .memoryChunk(offset.intValueSafe(), size.intValueSafe());
                ...
                case SUICIDE: {
                    ...
                    if (logger.isDebugEnabled()) {
                        hint = ADDRESS_LOG + Hex
                            .toHexString(program.getContractAddress().getLast20Bytes());
                    }
