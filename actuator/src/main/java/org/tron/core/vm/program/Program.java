...
public class Program {
            ...
            try {
                ...
                if (VMConfig.allowTvmConstantinople()) {
                    throw new TransferException(
                        "transfer all token or transfer all trx failed in suicide: %s",
                        e.getMessage());
                }
    ...
    private void createContractImpl(DataWord value, byte[] programCode, byte[] newAddress,
        ...
        if (VMConfig.allowTvmConstantinople()) {
            ...
            SmartContract newSmartContract = SmartContract.newBuilder()
                .setContractAddress(ByteString.copyFrom(newAddress))
                .setConsumeUserResourcePercent(100)
            ...
            if (afterSpend < 0) {
                createResult.setException(
                    Exception.notEnoughSpendEnergy("No energy to save just created contract code",
                        saveCodeEnergy,
                        programInvoke.getEnergyLimit() - createResult.getEnergyUsed()));
            } else {
        ...
        if (refundEnergy > 0) {
            ...
            if (logger.isDebugEnabled()) {
                logger.debug("The remaining energy is refunded, account: [{}], energy: [{}] ",
                    Hex.toHexString(
                        MUtil.convertToTronAddress(getContractAddress().getLast20Bytes())),
                    refundEnergy);
            }
        }
    }
    ...
    public void callToAddress(MessageCall msg) {
        ...
        if (byTestingSuite()) {
            ...
            if (!isTokenTransfer) {
                try {
                    VMUtils
                        .validateForSmartContract(deposit, senderAddress, contextAddress,
                            endowment);
                } catch (ContractValidateException e) {
                    if (VMConfig.allowTvmConstantinople()) {
                        refundEnergy(msg.getEnergy().longValue(),
                            "refund energy from message call");
        ...
        if (callResult != null) {
            ...
            if (BIUtil.isPositive(refundEnergy)) {
                refundEnergy(refundEnergy.longValueExact(),
                    "remaining energy from the internal call");
                if (logger.isDebugEnabled()) {
    ...
    public void refundEnergy(long energyValue, String cause) {
        logger
            .debug("[{}] Refund for cause: [{}], energy: [{}]", invoke.hashCode(), cause,
                energyValue);
    ...
    public void storageSave(DataWord word1, DataWord word2) {
        ...
        getContractState()
            .putStorageValue(MUtil.convertToTronAddress(getContractAddress().getLast20Bytes()),
                keyWord,
                valWord);
    }
    ...
    public byte[] getCodeAt(DataWord address) {
        byte[] code = invoke.getDeposit()
            .getCode(MUtil.convertToTronAddress(address.getLast20Bytes()));
    ...
    public void createContract2(DataWord value, DataWord memStart, DataWord memSize,
        DataWord salt) {
    ...
    public void callToPrecompiledAddress(MessageCall msg,
        ...
        if (!ArrayUtils.isEmpty(senderAddress) && !ArrayUtils.isEmpty(contextAddress)
            ...
            if (!isTokenTransfer) {
                ...
                try {
                    VMUtils
                        .validateForSmartContract(deposit, senderAddress, contextAddress, tokenId,
                            endowment);
                } catch (ContractValidateException e) {
                    throw new BytecodeExecutionException(VALIDATE_FOR_SMART_CONTRACT_FAILURE,
                        e.getMessage());
     ...
     * ---------------------------------------------------------------------------------------------
     * 0 allowed and only allowed                    error (guaranteed in CALLTOKEN) transfertoken
     * id=0 should not transfer trx） ---------------------------------------------------------------------
    ...
    public void checkTokenId(MessageCall msg) {
        if (VMConfig.allowMultiSign()) { //allowMultiSign proposal
            ...
            try {
                ...
                if (VMConfig.allowTvmConstantinople()) {
                    refundEnergy(msg.getEnergy().longValue(), "refund energy from message call");
                    throw new TransferException(VALIDATE_FOR_SMART_CONTRACT_FAILURE,
    ...
    /**
     * Denotes problem when executing Ethereum bytecode. From blockchain and peer perspective this
     * is quite normal situation and doesn't mean exceptional situation in terms of the program
     * execution
     */
    ...
    public static class Exception {
        ...
        public static OutOfEnergyException notEnoughSpendEnergy(String hint, long needEnergy,
            long leftEnergy) {
            return new OutOfEnergyException(
                "Not enough energy for '%s' executing: needEnergy[%d], leftEnergy[%d];", hint,
                needEnergy,
                leftEnergy);
        }
        ...
        public static OutOfEnergyException energyOverflow(BigInteger actualEnergy,
            ...
            return new OutOfEnergyException(
                "Energy value overflow: actualEnergy[%d], energyLimit[%d];",
        ...
        public static BadJumpDestinationException badJumpDestination(int pc) {
            return new BadJumpDestinationException("Operation with pc isn't 'JUMPDEST': PC[%d];",
                pc);
        }
